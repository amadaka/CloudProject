...
<title>Administrator class list </title>
...
<body>
  ...
  <div class="centered"><h1>Welcome ${loginU.first_name} ${loginU.last_name}</h1>
       <p>You Have Logged in as an Administrator</p>
        </div>
</div>
   </div>
   <div class="body">
  ...
  <a class="active2" href="courseController?action=adminupcominglist">Upcoming Courses</a>
</div>
    <div class="mission60004">
      <h2>List of Upcoming Classes</h2>
     <table class="center">
       <tr>
         <th>Class Number</th>
         <th>Event Name</th>
       ...
       <c:forEach items="${upCourse}" var="item" varStatus="status">
       <tr>
           <td>${item.id_class}</td>
           <td>${item.classname}</td>
           <td>${item.sdate}</td>
           <td>${item.edate}</td>
           <td><a onclick="return confirm(\'Are you sure?\')" href="courseController?action=SpecifyClass&id=${item.idcreate}">View</a></td>
           <td><a onclick="return confirm(\'Are you sure?\')" href="courseController?action=Redirectedit&id=${item.idcreate}">Edit</a></td>
           <td><a onclick="return confirm(\'Are you sure?\')" href="courseController?action=deleteClass&id=${item.idcreate}">Delete</a></td>
           </tr>
           </c:forEach>
     </table>
   </div>
