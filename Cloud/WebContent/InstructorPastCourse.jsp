...
<body>
  ...
  <a class="active2" href="courseController?action=teachpastlist">Your Past Courses</a>
  <a href="courseController?action=teachupcominglist">Your Upcoming Courses</a>
</div>
    <div class="mission60004">
      <h2>List of Courses you have taught in the past</h2>
     <table class="center">
       <tr>
         <th>Class Number</th>
         <th>Class Name</th>
        ...
        <c:forEach items="${pastinsCourse}" var="item" varStatus="status">
       <tr>
           <td>${item.id_class}</td>
           <td>${item.classname}</td>
           <td>${item.sdate}</td>
           <td>${item.edate}</td>
           <td><a onclick="return confirm(\'Are you sure?\')" href="courseController?action=SpecifyClassteachP&id=${item.idcreate}">View</a></td>
           </tr>
           </c:forEach>
     </table>
   </div>
