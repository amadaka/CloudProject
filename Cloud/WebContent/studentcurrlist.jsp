...
<body>
  ...
  <img class="logo" src="proyecto3/homepage-one-banner.jpg" alt="Snow">
   ...
   <div class="body">
       <div class="sidenav">
   <a class="active2" href="courseController?action=studentcurrlist">Your Current Courses</a>
  ...
  <a href="courseController?action=studentallupcominglist">Upcoming Courses</a>
    ...
    <div class="mission60004">
      <h2>List of Courses you are currently Taking</h2>
     <table class="center">
       <tr>
         <th>Class Number</th>
         <th>Class Name</th>
       ...
       <c:forEach items="${CurstuCourse}" var="item" varStatus="status">
       <tr>
           <td>${item.id_class}</td>
           <td>${item.classname}</td>
           <td>${item.sdate}</td>
           <td>${item.edate}</td>
           <td><a onclick="return confirm(\'Are you sure?\')" href="courseController?action=SpecifyClassStudP&id=${item.idcreate}">View</a></td>
           </tr>
           </c:forEach>
     </table>
   </div>
