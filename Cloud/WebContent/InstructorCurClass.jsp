...
<body>
  ...
  <div class="centered"><h1>Welcome ${loginU.first_name} ${loginU.last_name}</h1>
   ...
   <div class="body">
       <div class="sidenav">
  <a class="active2" href="courseController?action=teachcurrlist">Your Current Courses</a>
  ...
  <a href="courseController?action=teachupcominglist">Your Upcoming Courses</a>
    ...
    <div class="mission60004">
     ...
     <table class="center">
       ...
       <c:forEach items="${CurinsCourse}" var="item" varStatus="status">
       <tr>
           <td>${item.id_class}</td>
           <td>${item.classname}</td>
           <td>${item.sdate}</td>
           <td>${item.edate}</td>
           <td><a onclick="return confirm(\'Are you sure?\')" href="courseController?action=SpecifyClassteachC&id=${item.idcreate}">View</a></td>
           </tr>
           </c:forEach>
     </table>
   </div>
