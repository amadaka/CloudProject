...
<body>
  ...
  <a class="active2" href="courseController?action=teachupcominglist">Your Upcoming Courses</a>
</div>
    <div class="mission60004">
      <h2>List of Upcoming Courses you are going to teach</h2>
     <table class="center">
       <tr>
         <th>Class Number</th>
         <th>Class Name</th>
       ...
       <c:forEach items="${upinsCourse}" var="item" varStatus="status">
       <tr>
           <td>${item.id_class}</td>
           <td>${item.classname}</td>
           <td>${item.sdate}</td>
           <td>${item.edate}</td>
           <td><a onclick="return confirm(\'Are you sure?\')" href="courseController?action=SpecifyClassteachU&id=${item.idcreate}">View</a></td>
           </tr>
           </c:forEach>
     </table>
   </div>
